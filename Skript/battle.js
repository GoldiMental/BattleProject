...
async function self_attack(attack) {
    ...
    if (attack != "-") {
        ...
        if (tulpa_HP > 0) {
            ...
            console.log("Prüfe auf Level-Up: ", tulpa_self.XP, ">=", 10 * Math.round(1.35 ** tulpa_self.Lv), "? Ergebnis: ", tulpa_self.XP >= 10 * Math.round(1.35 ** tulpa_self.Lv)); //Level-Up-Prüfung
            if (tulpa_self.XP >= 10 * Math.round(1.35 ** tulpa_self.Lv)) {
                tulpa_self.Lv += 1;
                tulpa_self.XP = 0;
                tulpa_self.HP += 3;
